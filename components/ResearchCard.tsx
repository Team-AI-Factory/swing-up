...
export function ResearchCard({ id, snapshot }: { id: string; snapshot: ResearchSnapshot }) {
  return <article className="card research-card"><span className="research-badge" style={{ color: COLORS[snapshot.action] }}>{snapshot.label} · {snapshot.committeeApproved ? "Committee approved" : "Provisional alert"}</span><div><h2>{snapshot.ticker}</h2><p className="muted">{snapshot.company}</p></div><h3>{snapshot.title}</h3>

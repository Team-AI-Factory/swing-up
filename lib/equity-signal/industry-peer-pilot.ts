...
export type PilotHistoricalItem = {
  eventKey: string;
  ticker: string;
...
type DirectoryCache = { expiresAt: number; fetchedAt: string; profiles: Map<string, IndustryProfile>; pagesRequested: number; pagesFailed: number; errors: string[] };
...
function emptyDiagnostics(cacheUsed = false): PilotHistoricalGate["directoryDiagnostics"] {
  return { provider: "TradingView public US stock scanner", fetchedAt: null, primaryListings: 0, pagesRequested: 0, pagesFailed: 0, errors: [], cacheUsed };
}
...
function parseProfile(value: unknown): IndustryProfile | null {
  ...
  if (!ticker || text(data[4]) !== "stock" || !boolean(data[5])) return null;
...
async function fetchDirectoryPage(fetchImpl: typeof fetch, start: number) {
  try {
    const response = await fetchImpl("https://scanner.tradingview.com/america/scan", {
      method: "POST",
      cache: "no-store",
      headers: { accept: "application/json", "content-type": "application/json", origin: "https://www.tradingview.com", referer: "https://www.tradingview.com/", "user-agent": "Mozilla/5.0 (compatible; SwingUpIndustryPeers/2.0)" },
      body: JSON.stringify({
        filter: [{ left: "type", operation: "equal", right: "stock" }, { left: "is_primary", operation: "equal", right: true }],
        options: { lang: "en" },
        markets: ["america"],
    ...
    return { totalCount: Math.max(rows.length, Math.floor(finite(container.totalCount) ?? rows.length)), profiles: rows.flatMap((row) => parseProfile(row) ?? []), error: null as string | null };
  } catch (error) {
...
async function industryDirectory(fetchImpl: typeof fetch) {
  ...
  const pages = [first, ...await mapWithConcurrency(starts, 6, (start) => fetchDirectoryPage(fetchImpl, start))];
  ...
  directoryCache = { expiresAt: Date.now() + CACHE_MS, fetchedAt: new Date().toISOString(), profiles, pagesRequested: pages.length, pagesFailed: pages.filter((page) => page.error).length, errors: pages.flatMap((page) => page.error ?? []) };
  return { cache: directoryCache, cacheUsed: false };
}

function exactCompanyItems(candidate: Json): PilotHistoricalItem[] {
  const ticker = text(candidate.ticker)?.toUpperCase() ?? "";
  const analog = object(candidate.historicalAnalog);
  return (Array.isArray(analog.items) ? analog.items : []).flatMap((raw): PilotHistoricalItem[] => {
    ...
    if (!ticker || itemTicker !== ticker || !eventKey || !horizon || returnPercent === null || !text(provenance.eventSourceUrl) || !text(provenance.priceSource)) return [];
    return [{ eventKey, ticker: itemTicker, signalObservedAt: text(item.signalObservedAt) ?? "", outcomeObservedAt: text(item.outcomeObservedAt) ?? "", horizon, directionAdjustedReturnPercent: returnPercent, hit: item.hit === true, sector: null, industry: null, matchType: "exact_company", eventSourceUrl: text(provenance.eventSourceUrl) ?? "", priceSource: text(provenance.priceSource) ?? "" }];
  });
}
...
function summarize(mode: PilotHistoricalGate["mode"], items: PilotHistoricalItem[], horizon: HistoricalAnalogHorizon | null, sector: string | null, industry: string | null, directory: PilotHistoricalGate["directoryDiagnostics"]): PilotHistoricalGate {
  const unique = [...new Map(items.map((item) => [item.eventKey, item])).values()];
  const hitRate = unique.length ? (unique.filter((item) => item.hit).length / unique.length) * 100 : 0;
  ...
  const peerMode = mode === "same_industry_peer_history" || mode === "same_sector_peer_history";
  const checks = {
    ...
    usableHistoricalHorizon: horizon !== null,
    nonNegativeLowerQuartileOutcome: lowerQuartile !== null && lowerQuartile >= 0,
    sameCompanyOrIndustryPeer: mode === "exact_company_history" || (peerMode && distinctTickers >= 2),
  };
  return {
    ...
    blockers: [
      ...
      ...(!checks.leakageSafeHistory ? ["At least one event failed the no-future-information check."] : []),
    ...
    items: unique,
    directoryDiagnostics: directory,
...
function eligibleRecord(record: HistoricalSignalRecord, candidate: Json, now: Date) {
  ...
  return record.dataQuality === "real"
    && record.eventFamily === text(candidate.eventFamily)
    && record.direction === text(candidate.direction)
...
export async function evaluateIndustryPeerPilotGate(input: { candidate: unknown; historicalSignals: HistoricalSignalRecord[]; fetchImpl?: typeof fetch; now?: Date }) {
  const candidate = object(input.candidate);
  const now = input.now ?? new Date();
  const fetchImpl = input.fetchImpl ?? fetch;
  const ticker = text(candidate.ticker)?.toUpperCase() ?? "";
  if (!ticker || !text(candidate.eventFamily) || !text(candidate.direction)) return summarize("insufficient_history", [], null, null, null, emptyDiagnostics());
  ...
  const exactHorizon = horizons.find((horizon) => exact.filter((item) => item.horizon === horizon).length >= MINIMUM_EVENTS) ?? null;
  if (exactHorizon) return summarize("exact_company_history", exact.filter((item) => item.horizon === exactHorizon), exactHorizon, null, null, emptyDiagnostics(true));
  ...
  const diagnostics: PilotHistoricalGate["directoryDiagnostics"] = { provider: "TradingView public US stock scanner", fetchedAt: directory.fetchedAt, primaryListings: directory.profiles.size, pagesRequested: directory.pagesRequested, pagesFailed: directory.pagesFailed, errors: directory.errors, cacheUsed: directoryResult.cacheUsed };
  if (!current || (!current.industry && !current.sector)) return summarize("insufficient_history", [], null, current?.sector ?? null, current?.industry ?? null, diagnostics);

  const records = input.historicalSignals.filter((record) => eligibleRecord(record, candidate, now));
  let bestItems: PilotHistoricalItem[] = [];
  let bestHorizon: HistoricalAnalogHorizon | null = null;
  let bestMode: PilotHistoricalGate["mode"] = "insufficient_history";
  for (const horizon of horizons) {
    const sameIndustry: PilotHistoricalItem[] = [];
    const sameSectorFallback: PilotHistoricalItem[] = [];
    for (const record of records) {
      const profile = directory.profiles.get(record.ticker.toUpperCase());
      const checkpoint = record.checkpoints[horizon];
      if (!profile || !checkpoint || Date.parse(checkpoint.observedAt) >= now.getTime()) continue;
      const industryMatch = Boolean(current.industry && profile.industry && normalized(current.industry) === normalized(profile.industry));
      const sectorFallbackMatch = Boolean(!current.industry && current.sector && profile.sector && normalized(current.sector) === normalized(profile.sector));
      if (!industryMatch && !sectorFallbackMatch) continue;
      const adjusted = record.direction === "downside" ? -checkpoint.returnPercent : checkpoint.returnPercent;
      const item: PilotHistoricalItem = { eventKey: record.eventKey, ticker: record.ticker.toUpperCase(), signalObservedAt: record.signalObservedAt, outcomeObservedAt: checkpoint.observedAt, horizon, directionAdjustedReturnPercent: Math.round(adjusted * 100) / 100, hit: adjusted > 0, sector: profile.sector, industry: profile.industry, matchType: industryMatch ? "same_industry" : "same_sector", eventSourceUrl: record.provenance?.eventSourceUrl ?? "", priceSource: record.provenance?.priceSource ?? "" };
      (industryMatch ? sameIndustry : sameSectorFallback).push(item);
    }
    const industryUnique = [...new Map(sameIndustry.map((item) => [item.eventKey, item])).values()];
    const sectorUnique = [...new Map(sameSectorFallback.map((item) => [item.eventKey, item])).values()];
    const selected = industryUnique.length ? industryUnique : sectorUnique;
    const mode: PilotHistoricalGate["mode"] = industryUnique.length ? "same_industry_peer_history" : sectorUnique.length ? "same_sector_peer_history" : "insufficient_history";
    if (selected.length > bestItems.length) { bestItems = selected; bestHorizon = horizon; bestMode = mode; }
    if (selected.length >= MINIMUM_EVENTS) { bestItems = selected; bestHorizon = horizon; bestMode = mode; break; }
  }
  return summarize(bestMode, bestItems, bestHorizon, current.sector, current.industry, diagnostics);
}

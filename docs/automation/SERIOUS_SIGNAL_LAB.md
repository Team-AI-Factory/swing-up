...
- at least five independent real historical events supporting the same proposed direction;
- opposite-direction events excluded from the pilot sample;
...
The public bootstrap uses official issuer announcements and rebuilds stock and SPY outcomes from public adjusted daily history at runtime. It stores no hard-coded return or future outcome. Every new qualified finding is stored in R2 and measured at 1D, 3D, 7D, 30D, and 90D so the evidence set grows over time. Pending findings and completed checkpoint outcomes are stored separately and immutably.

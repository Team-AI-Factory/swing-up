...
export async function runPilotEquitySignalLab(input: PilotEquitySignalLabInput = {}) {
  ...
  const common = {
    ...
    historicalLearning: {
      ...
      historicalEvidenceRole: "required_five_case_pilot_gate_for_directional_serious_alerts",
  ...
  if (!directionalAction) {
    return {
      ...common,
      status: "candidate_not_certified_directional_buy_or_sell",
      seriousSignalFound: false,
      actionableSignalFound: false,
      alertType: null,
      blockers: [...new Set([...strings(base.blockers), "Event-first Watch alerts are not an active Watch Out rule. Only the separately certified extreme-volatility rule may emit Watch Out."])],
    };
  }

...
export async function runAiCommittee(input: RunAiCommitteeInput) {
  ...
  if (!evidence.ok || !evidence.evidencePack) return { ok: false, status: evidence.error ?? "evidence_pack_missing", dryRun, evidence };
  if (!dryRun && evidence.missingRequiredEvidence.length) return { ok: false, status: "evidence_pack_incomplete", dryRun, missingRequiredEvidence: evidence.missingRequiredEvidence, evidence };
  ...
  if (!dryRun && estimatedCost > maxCostUsd) return { ok: false, status: "cost_limit_exceeded", dryRun, estimatedCost, maxCostUsd };
  ...
  return { ok: true, status: dryRun ? "dry_run" : "completed", dryRun, mode, providerStatus: dryRun ? { ...providerStatus, openAiCalled: false } : providerStatus, plannedAgents: agents.map((agent) => agent.id).concat(finalJudge ? [finalJudge.id] : []), evidence, agentResults, committeeOutput, compatibility: { callsOpenAi: !dryRun, publishes: false, sendsTelegram: false, writesDatabase: false } };
}

...
const completedFinding = {
  ...pendingFinding,
  provenance: { ...pendingFinding.provenance, origin: "swing_up_forward_outcome" },
  checkpoints: { "1D": first.records[0].checkpoints["1D"] },
};
const transitioned = mergeHistoricalSignals([pendingFinding], [completedFinding]);
assert.equal(transitioned.length, 1);
assert.equal(transitioned[0].provenance.origin, "swing_up_forward_outcome");
assert.ok(transitioned[0].checkpoints["1D"]);
...
console.log(JSON.stringify({
  ...
  pendingFindingTransitionsToCompletedOutcome: true,
}, null, 2));

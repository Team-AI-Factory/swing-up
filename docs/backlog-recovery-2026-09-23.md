...
- The per-cycle admission ceiling rises from 12 to 24 for large queues. Processing remains sequential under the existing lease, deadline and cost safeguards. This is an upper bound, not a promised measured throughput.
- Profile maintenance selects up to twelve companies instead of two, with two concurrent issuers and request starts paced at 250 ms. Fresh queued issuers receive first priority, then current valuation candidates, and one background selection remains available. The 30-second maintenance deadline now also aborts requests waiting to start. Internal daily SEC metadata ceilings rise from 190 to 1,200 and the shared SEC metadata/facts ceiling from 400 to 2,400; these are application ceilings for free public data, not a purchased entitlement. Commercial provider caps are unchanged. SEC fair access still applies (https://www.sec.gov/about/developer-resources).
- The independent paid-review count ceiling rises from 20 to 60 per rolling day. The existing $10 rolling cost cap, $6 warning, reservation accounting and per-evidence cooldown remain authoritative. The change does not add budget or authorize calls without evidence.
...
- Compact logs include per-cycle deferral counts and per-event blocker details. The result list can retain all 24 admitted attempts instead of silently cutting diagnostics at twelve. Profile-ready queue age is also reported; legacy rows without firstQueuedAt use source time, not an invented queue-entry time.

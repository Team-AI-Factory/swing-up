...
export function branchProviderCallRequest(value: RequestInfo | URL, now: Date): BranchProviderCallRequest | null {
  const url = requestUrl(value);
  if (!url) return null;
...
export async function runBranchSignalLab(input: BranchSignalLabInput = {}) {
  ...
  const quotaAwareFetch: typeof fetch = async (request, init) => {
    ...
    if (grantKey) {
      ...
      if (!requestedDirectory || requestedDirectory !== allowance.archiveDirectory) throw new Error("sec_filing_detail_grant_scope_mismatch");
    ...
    const quotaRequest = branchProviderCallRequest(request, now);
    if (quotaRequest && input.beforeProviderCall) {
      const decision = await input.beforeProviderCall(quotaRequest);
      if (!decision.allowed) throw new ProviderBudgetError(quotaRequest.provider, `${quotaRequest.provider}_${decision.reason}`, decision.nextRetryAt);
    }
    return rawFetch(request, rawInit);
  };
  const reserveSecFilingDetailAccessions: ReserveSecFilingDetailAccessions = async (requests) => {
    const quotaRequests = requests.map((request) => {
      const quotaRequest = branchProviderCallRequest(request.indexUrl, now);
      if (!quotaRequest || quotaRequest.quotaKey !== "sec_filing_details") throw new Error("sec_filing_detail_grant_request_invalid");
    ...
    const decisions = input.beforeProviderCall
      ...
      : quotaRequests.map(() => ({ allowed: true, nextRetryAt: null, reason: "reserved" as const }));
    decisions.forEach((decision, index) => {
      ...
      secFilingDetailGrantAllowances.set(request.filingKey, { remaining: request.maximumRequests, archiveDirectory });
    });
    return decisions;
  };
  return runPilotEquitySignalLab({
    ...input,
    now,

...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ ok: true, sanitized: true, coverage: valuation.foundation.coverage, dataQuality, generatedAt: new Date().toISOString(), sort: "buy_and_sell_by_base_case_potential_desc", total: alerts.length, truncated: alerts.length > 200, alerts: alerts.slice(0, 200) },

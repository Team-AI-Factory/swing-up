...
export async function runApprovalGate(input: ApprovalGateInput) {
  ...
  checks.push(
    ...
    { key: "live_score_inputs", label: "Scoring uses complete live evidence", required: true, passed: latestScore?.liveDataReady === true && scoreNumber(latestScore, "inputCompleteness") === 100, detail: latestScore?.liveDataReady === true ? "All required scoring inputs have live provenance." : `Live scoring inputs are incomplete: ${JSON.stringify(latestScore?.missingInputs ?? [])}.` },
    ...
    { key: "historical_pattern", label: "Optional historical context status recorded (available or missing)", required: true, passed: Boolean(pack && (pack.historicalPatternMatch.available || pack.historicalPatternMatch.summary?.toLowerCase().includes("not available"))), detail: pack?.historicalPatternMatch.summary ?? "Historical context status unavailable." },

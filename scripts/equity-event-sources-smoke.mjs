...
// PR262 keeps its Pilot-5 wrapper. These focused tests prove the shared-source
// fallbacks themselves; provider-budget routing belongs to the wrapper's own
// smoke suite and is intentionally not transplanted from PR261 here.
assert.match(quotaSource, /runPilotEquitySignalLab/);
assert.match(quotaSource, /input\.beforeProviderCall/);
assert.match(
  source,
...
globalThis.__swingUpEquityQuotes?.delete("STAL");
globalThis.__swingUpEquityQuotes?.delete("SPY");
const quoteSuccessAt = new Date("2026-07-23T14:00:00.000Z");
const freshQuoteCandidate = [{ ticker: "STAL", direction: "upside", pricedInPenalty: 0, gatePassed: true, trackingDisposition: "qualified", quote: null, rootEventKey: "stale-quote-event" }];
await marketLoaded.exports.enrichCandidateQuotes(freshQuoteCandidate, async (value) => {
  const ticker = new URL(String(value)).pathname.split("/").at(-1);
  const price = ticker === "SPY" ? 600 : 25;
  return new Response(JSON.stringify({
    chart: {
      result: [{
        meta: {
          regularMarketPrice: price,
          chartPreviousClose: price,
          regularMarketTime: Date.parse("2026-07-23T13:59:00.000Z") / 1000,
        },
        timestamp: [Date.parse("2026-07-23T13:59:00.000Z") / 1000],
      }],
      error: null,
    },
  }), { status: 200 });
...
const staleQuoteResult = await marketLoaded.exports.enrichCandidateQuotes(staleQuoteCandidate, async (value) => {
  outageCalls.push(String(value));
  throw new Error("quote provider outage");
}, new Date(quoteSuccessAt.getTime() + 16 * 60 * 1000));
assert.equal(staleQuoteCandidate[0].quote?.price, 25);
assert.equal(staleQuoteCandidate[0].quote?.providerFetchedAt, originalProviderFetchedAt);
assert.equal(staleQuoteCandidate[0].quote?.cacheAgeMs, 16 * 60 * 1000);
assert.equal(staleQuoteCandidate[0].quote?.actionableForSeriousSignal, false);
assert.equal(staleQuoteResult.provider.status, "temporarily_unavailable");
assert.equal(staleQuoteResult.provider.cached, true);

const callsBeforeRepeatedOutage = outageCalls.length;
const repeatedOutageCandidate = [{ ticker: "STAL", direction: "upside", pricedInPenalty: 0, gatePassed: true, trackingDisposition: "qualified", quote: null, rootEventKey: "stale-quote-event" }];
await marketLoaded.exports.enrichCandidateQuotes(repeatedOutageCandidate, async (value) => {
  outageCalls.push(String(value));
  throw new Error("quote provider outage");
}, new Date(quoteSuccessAt.getTime() + 17 * 60 * 1000));
assert.equal(outageCalls.length, callsBeforeRepeatedOutage);
assert.equal(repeatedOutageCandidate[0].quote?.providerFetchedAt, originalProviderFetchedAt);
assert.equal(repeatedOutageCandidate[0].quote?.cacheAgeMs, 17 * 60 * 1000);
assert.equal(repeatedOutageCandidate[0].quote?.actionableForSeriousSignal, false);

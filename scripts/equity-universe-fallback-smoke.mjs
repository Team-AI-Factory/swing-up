...
const stubs = {
  "@/lib/r2-warehouse": {
    ...
    readVersionedTextFromR2: async (key) => {
      ...
      return value
        ? { found: true, text: JSON.stringify(value), etag: `etag-${key}` }
        : { found: false, text: null, etag: null };
    },
    writeVersionedJsonToR2: async (key, value) => {
      r2WriteKeys.push(key);
      if (r2Objects.has(key)) return { written: false, conflict: true, etag: `etag-${key}` };
      r2Objects.set(key, value);
      return { written: true, conflict: false, etag: `etag-${key}` };
    },
  },
...
const PR_261_CACHE_KEY = "branch-labs/pr-261/equity-universe/v1.json";
const PR_262_CACHE_KEY = "branch-labs/pr-262/equity-universe/v1.json";
assert.equal(resolveEquityUniverseCacheKey(PR_261_BRANCH), PR_261_CACHE_KEY);
assert.equal(resolveEquityUniverseCacheKey(PR_262_BRANCH), PR_262_CACHE_KEY);
assert.notEqual(resolveEquityUniverseCacheKey(PR_261_BRANCH), resolveEquityUniverseCacheKey(PR_262_BRANCH));
assert.equal(resolveEquityUniverseCacheKey("agent/local-test"), null);
assert.equal(resolveEquityUniverseCacheKey(""), null);
...
const originalRailwayBranch = process.env.RAILWAY_GIT_BRANCH;
r2Configured = true;
process.env.RAILWAY_GIT_BRANCH = PR_261_BRANCH;
const pr261Universe = await loadEquityUniverse(secOnlyFetch, new Date("2026-07-22T13:25:00.000Z"));
assert.equal(pr261Universe.r2Write, true);
assert.equal(r2WriteKeys.at(-1), PR_261_CACHE_KEY);
assert.ok(r2Objects.has(PR_261_CACHE_KEY));
assert.equal(r2Objects.has(PR_262_CACHE_KEY), false);

process.env.RAILWAY_GIT_BRANCH = PR_262_BRANCH;
const pr262Universe = await loadEquityUniverse(secOnlyFetch, new Date("2026-07-22T13:30:00.000Z"));
assert.equal(pr262Universe.r2Write, true);
assert.equal(r2WriteKeys.at(-1), PR_262_CACHE_KEY);
assert.ok(r2Objects.has(PR_261_CACHE_KEY));
assert.ok(r2Objects.has(PR_262_CACHE_KEY));
assert.equal(new Set(r2WriteKeys).size, 2);

const readsBeforeUnknownBranch = r2ReadKeys.length;
const writesBeforeUnknownBranch = r2WriteKeys.length;
process.env.RAILWAY_GIT_BRANCH = "agent/local-test";
const localUniverse = await loadEquityUniverse(secOnlyFetch, new Date("2026-07-22T13:35:00.000Z"));
assert.equal(localUniverse.r2Write, false);
assert.equal(r2ReadKeys.length, readsBeforeUnknownBranch);
assert.equal(r2WriteKeys.length, writesBeforeUnknownBranch);
if (originalRailwayBranch === undefined) delete process.env.RAILWAY_GIT_BRANCH;
else process.env.RAILWAY_GIT_BRANCH = originalRailwayBranch;
...
console.log(JSON.stringify({
  ...
  branchSpecificR2CacheKeys: true,
  unknownBranchesCannotShareR2UniverseCache: true,

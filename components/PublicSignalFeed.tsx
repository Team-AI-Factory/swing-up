...
export function PublicSignalFeed() {
  ...
  useEffect(() => {
    let active = true;
    const load = async () => { try { const response = await fetch("/api/public/signals"); const body = await response.json();
  ...
  return <section id="live-alerts"><h2>Live opportunities and risk alerts</h2>
    ...
    <div className="grid">{visible.slice(0, visibleCount).map(s => <article className="card alert-card" key={s.id}>
      ...
      <p>Recorded price: {typeof s.currentPrice === "number" ? s.currentPrice.toLocaleString("en-US", { maximumFractionDigits: 2 }) : "Still being collected"} · Model base value: {typeof s.fairValue === "number" ? s.fairValue.toLocaleString("en-US", { maximumFractionDigits: 2 }) : "Not established"}</p>
      ...
      <div className="button-row">{s.sources.map(source => <a key={source.url} href={source.url} target="_blank" rel="noreferrer">{source.label || "Evidence"}</a>)}</div>
    </article>)}</div>

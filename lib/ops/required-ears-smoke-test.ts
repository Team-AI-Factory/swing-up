...
async function runOne(source: RequiredEar, dryRun: boolean, perSourceLimit: number): Promise<SourceResult> {
  try {
    ...
    if (source === "google-news") { const r = await runGoogleNewsRssIngestion({ dryRun }); return result(source, r.sourceHealthStatus === "error" ? "failed" : r.sourceHealthStatus, r.articlesChecked, r.rawSignalsCreated, r.sourceHealthStatus === "error" ? r.errors : [], r.sourceHealthStatus === "degraded" ? r.errors : [], "Public RSS feed checked; no API key required."); }

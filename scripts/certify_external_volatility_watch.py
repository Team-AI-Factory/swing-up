#!/usr/bin/env python3
"""External certification for a frozen extreme-volatility Watch Out alert.

This is deliberately non-directional: it warns that a severely damaged security
is likely to make another very large move. It must pass on a completely unused
stock universe before Swing Up may label it a serious alert.
"""
...
def main() -> int:
    ...
    if overlap:
        raise RuntimeError(f"External volatility holdout overlaps a used ticker: {overlap}")
    rows = []
    ...
    report = {
        ...
        "certificationPosture": "serious_extreme_volatility_watch_out_certified" if passed else "external_volatility_holdout_failed_alert_remains_research_only",
        "reusePolicy": "This external holdout is burned after this run and must never be used to tune the rule.",
...
if __name__ == "__main__":
    try:
        ...
        failure = {
            ...
            "certificationPosture": "external_volatility_holdout_invalid_or_unavailable",

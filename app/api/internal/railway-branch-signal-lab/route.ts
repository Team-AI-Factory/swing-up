...
function normalizeHistory(parsed: History) {
  ...
  parsed.deploymentId = process.env.RAILWAY_DEPLOYMENT_ID?.trim() || null;
...
async function loadHistory(): Promise<{ history: History; storage: StateStorage }> {
  // PR #262 must never seed itself from PR #261's legacy file or Railway
  // volume. Its R2 namespace starts independently.
  const legacy = ACTIVE_BRANCH === COMBINED_BRANCH
    ? { history: emptyHistory(), source: null }
    : await readLegacyHistory(legacyStorage()).catch(() => ({ history: emptyHistory(), source: null }));
...
function historicalSignalRecords(history: History): HistoricalSignalRecord[] {
  ...
  return qualifiedFindingEntries(history).flatMap((finding) => {
    ...
    const origin = firstCheckpoint ? "swing_up_forward_outcome" : "swing_up_tracked_finding";
    return [{
...
function hasCompletedCheckpoint(item: HistoricalSignalRecord) {
  return Object.values(item.checkpoints).some(Boolean);
}

function isSwingUpTrackedFinding(item: HistoricalSignalRecord) {
  return item.provenance?.origin === "swing_up_tracked_finding"
    || (item.provenance?.origin === "swing_up_forward_outcome" && !hasCompletedCheckpoint(item));
}
...
function runArchiveKey(report: JsonRecord, invocation: SchedulerInvocation) {
  const rawCheckedAt = typeof report.checkedAt === "string" && Number.isFinite(Date.parse(report.checkedAt))
    ? report.checkedAt
    : new Date().toISOString();
  const date = rawCheckedAt.slice(0, 10);
  const timestamp = rawCheckedAt.replace(/[^0-9]/g, "").slice(0, 17);
  const worker = invocation.workerId.toLowerCase().replace(/[^a-z0-9_-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 80) || "worker";
  return `${R2_RUN_ARCHIVE_PREFIX}/${date}/${timestamp}-${worker}-seq-${invocation.sequence}.json`;
}
...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    historicalOutcomeLibrary: {
      ...
      swingUpTrackedFindingRecordCount: historicalLibrary.library.records.filter(isSwingUpTrackedFinding).length,
      swingUpCompletedForwardOutcomeRecordCount: historicalLibrary.library.records.filter(isSwingUpCompletedOutcome).length,
...
async function executePost(request: NextRequest) {
  ...
  const forwardOutcomeAdditions = historicalSignalRecords(historyIncludingCurrentRun).map((item): HistoricalSignalRecord => ({
    ...
    provenance: item.provenance ?? {
      ...
      eventSourceUrl: `r2://branch-labs/${R2_BRANCH_NAMESPACE}/serious-signal/state.json`,
  ...
  report.historicalLearning = {
    ...
    r2LibrarySwingUpTrackedFindingRecordCount: historicalLibrary.library.records.filter(isSwingUpTrackedFinding).length,
  ...
  if (!archived.written) throw new Error("r2_run_archive_write_conflict");

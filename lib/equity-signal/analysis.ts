...
function currentEvidenceGateScore(input: {
  eventTruth: number;
  ...
  return computeEventFirstStrength({ ...input, historicalSupport: 0 });
}
...
function candidateFromCluster(cluster: MappedEvent[], macro: MacroContext, historicalSignals: HistoricalSignalRecord[], now: Date): ImpactCandidate {
  ...
  const currentEvidenceScore = currentEvidenceGateScore({ eventTruth, mappingConfidence, materiality, transmissionConfidence, evidenceIndependence, contradictionPenalty: contradiction, pricedInPenalty, rumour });
  ...
  return {
    ...
    gateChecks: {
      ...
      currentEvidenceStrengthPassed: currentEvidenceScore >= 72,
    },
    gatePassed: gate.passed
      && currentEvidenceScore >= 72

...
function aggregateProviderRows(rows: ProviderResult[]) {
  ...
  return [...grouped.entries()].map(([provider, providerRows]) => {
    ...
    return result({
      ...
      receipts: selectEquityReceipts(providerRows.flatMap((row) => row.receipts), 300),
...
export async function fetchSecCurrentFilings(fetchImpl: typeof fetch, now: Date): Promise<ProviderResult> {
  ...
  return result({
    ...
    receipts: selectEquityReceipts(successes.flatMap((item) => item.value.receipts), 300),
...
export async function fetchGdeltDiscovery(fetchImpl: typeof fetch, now: Date): Promise<ProviderResult> {
  ...
  try {
    ...
    return result({ provider: "gdelt", status: "connected", checkedAt: now.toISOString(), sourceUrls: [url.toString()], receipts: selectEquityReceipts(receipts, 200), recordsRead: receipts.length, entitlementVerified: true });
  } catch (error) {
    const failure = errorCategory(error);
...
export async function fetchAlphaNews(fetchImpl: typeof fetch, now: Date): Promise<ProviderResult> {
  ...
  try {
    ...
    return result({ provider: "alpha_vantage_news", status: "connected", checkedAt: now.toISOString(), sourceUrls: [`${ALPHA_VANTAGE_URL}?function=NEWS_SENTIMENT`], receipts: selectEquityReceipts(receipts, 200), recordsRead: receipts.length, entitlementVerified: true });
  } catch (error) {
    const failure = errorCategory(error);
...
export async function collectEventSources(
  ...
  const baseReceipts = selectEquityReceipts(baseProviders.flatMap((provider) => provider.receipts), 500);
  ...
  const receipts = selectEquityReceipts(mergeSecFilingDetails(baseReceipts, detailResult.details), 500);

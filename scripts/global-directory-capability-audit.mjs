...
const outputPath = process.env.GLOBAL_DIRECTORY_AUDIT_PATH || "artifacts/global-directory-capability-audit.json";
...
async function request(path) {
  ...
  let json;
  try { json = JSON.parse(raw); } catch { throw new Error(`Expected JSON; status=${response.status}; body=${raw.slice(0, 400)}`); }
...
while (Date.now() < deadline) {
  ...
  } catch { /* deployment may still be rolling */ }
  await sleep(10_000);
}
assert.ok(health, `Expected Railway commit was not available: ${expectedCommit}`);

const audit = await request("/api/internal/combined-opportunity-engine/global-directory-audit");
...
console.log(JSON.stringify({
  ...
  usableEndpoints: report.usableEndpoints,
  endpointResults: report.results.map((row) => ({ endpoint: row.endpoint, status: row.status, rowCount: row.rowCount })),

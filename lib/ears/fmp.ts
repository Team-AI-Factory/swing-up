...
export const DEFAULT_FMP_TICKERS = ["AAPL", "MSFT", "NVDA"] as const;

const FMP_BASE_URL = "https://financialmodelingprep.com/api/v3";
const DEFAULT_LIMIT = DEFAULT_FMP_TICKERS.length;
const MAX_LIMIT = DEFAULT_FMP_TICKERS.length;

type FmpRunOptions = { dryRun?: boolean; limit?: number; tickers?: string[] };
type FmpEventType = "price_movement" | "earnings_event" | "analyst_target_change" | "fundamentals_change" | "transcript_signal";
type FmpCandidate = {
  ticker: string;
...
async function updateFmpSourceHealth(status: "connected" | "not_configured" | "degraded" | "error", startedAt: number, errorMessage: string | null) {
  ...
  await prisma.sourceHealth.upsert({
    ...
    create: { source: FMP_SOURCE, status, checkedAt: now, lastSuccessAt: status === "connected" || status === "degraded" ? now : null, responseTimeMs: Date.now() - startedAt, errorMessage, usage: "Financial Modeling Prep paid market/fundamental ear", notes: "Uses FMP_API_KEY for small-sample price, fundamentals, earnings, analyst target, and transcript-related raw signals. Creates raw signals only; never final alerts." },
    update: { status, checkedAt: now, lastSuccessAt: status === "connected" || status === "degraded" ? now : undefined, responseTimeMs: Date.now() - startedAt, errorMessage, usage: "Financial Modeling Prep paid market/fundamental ear", notes: "Uses FMP_API_KEY for small-sample price, fundamentals, earnings, analyst target, and transcript-related raw signals. Creates raw signals only; never final alerts." },
  });
  return status;
}
...
function analystCandidate(ticker: string, row: Record<string, unknown>): FmpCandidate | null {
  ...
  return { ticker, eventType: "analyst_target_change", title: `${ticker} FMP analyst target snapshot`, summary: `${ticker} FMP analyst target sample shows a consensus/average target near $${target}. This is raw analyst-target context only.`, url: `https://financialmodelingprep.com/financial-summary/${ticker}`, detectedAt, duplicateKey: `${FMP_SOURCE}|analyst_target_change|${ticker}|${detectedAt.slice(0, 10)}`, importanceHint: "low", payload: { provider: FMP_SOURCE, endpoint: "price-target-consensus", row: row as Prisma.InputJsonObject, noFinalAlerts: true } };
}
...
export async function runFmpIngestion(options: FmpRunOptions = {}): Promise<FmpRunResult> {
  ...
  try {
    ...
    for (const ticker of tickers) {
      const [quote, fundamentals, earnings, analyst, transcripts] = await Promise.allSettled([

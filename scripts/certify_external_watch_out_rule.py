#!/usr/bin/env python3
"""One-time external certification for the frozen Swing Up Watch Out rule.

This script must run only on tickers excluded from the development universe.
The rule is fixed before the external outcomes are examined. Failure means the
rule remains research-only and this same holdout must never be used for tuning.
"""
...
DATASET_PATH = Path(os.getenv("EXTERNAL_WATCH_OUT_DATASET_PATH", "artifacts/external-watch-out-dataset.json"))
REPORT_PATH = Path(os.getenv("EXTERNAL_WATCH_OUT_REPORT_PATH", "artifacts/external-watch-out-certificate.json"))
MIN_SIGNALS = max(30, int(os.getenv("EXTERNAL_WATCH_OUT_MIN_SIGNALS", "30")))
TARGET_ERROR = float(os.getenv("EXTERNAL_WATCH_OUT_TARGET_ERROR", "0.10"))
CERTIFICATE_CONFIDENCE = float(os.getenv("EXTERNAL_WATCH_OUT_CERTIFICATE_CONFIDENCE", "0.90"))
MIN_UNIQUE_TICKERS = max(15, int(os.getenv("EXTERNAL_WATCH_OUT_MIN_UNIQUE_TICKERS", "20")))

RULE = {
    "id": "watch_out_90d_extreme_volatility_high_beta_v1",
    "action": "watch_out",
...
def main() -> int:
    ...
    overlap = sorted(requested & DEVELOPMENT_TICKERS)
    if overlap:
        raise RuntimeError(f"External holdout overlaps development tickers: {overlap}")
    rows = []
    for item in payload.get("rows", []):
        ...
        if not all(isinstance(value, (int, float)) for value in [volatility, beta60, drawdown, return90]):
            continue
        if volatility >= RULE["volatility60dMinimum"] and beta60 >= RULE["beta60dMinimum"]:
            rows.append({
    ...
    if selected.empty:
        selected = pd.DataFrame(columns=["ticker", "eventDate", "volatility60d", "beta60d", "drawdown90d", "return90d", "won"])
    ...
    overall_pass = bool(
        ...
        and overall["observedPrecision"] is not None
        and overall["observedPrecision"] >= 0.90
    ...
    report = {
        ...
        "certificationPosture": "serious_watch_out_alert_certified" if passed else "external_holdout_failed_rule_remains_research_only",
        "reusePolicy": "This external holdout is burned after this run and must not be used to tune thresholds or model features.",
...
if __name__ == "__main__":
    try:
        ...
        failure = {
            ...
            "certificationPosture": "external_holdout_invalid_or_unavailable",

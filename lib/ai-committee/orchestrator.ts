...
function buildAgentPrompt(agent: AiCommitteeAgentDefinition, evidencePack: AiCommitteeEvidencePack, previousResults: AiCommitteeAgentResult[], mode: AiCommitteeMode) {
  ...
  const assetInstructions = evidencePack.analysisKind === "valuation"
    ? "This is a company-first valuation review. No new headline, catalyst or price move is required. Assess the dated financial statements, sustainable earnings and cash flow, debt, share count, sector-appropriate valuation methods, assumptions, valuation range, margin of safety and current price. Model value is an estimate, never a confirmed fact or promised return. Request only data actually required to assess this company. Missing news, FDA, macro or event exhibits are N/A unless relevant to this valuation."
  ...
  const discoveryProviderInstructions = evidencePack.analysisKind === "valuation"
    ? "For this valuation, verify the supplied financial sources, reporting periods and model assumptions. News-publisher counts and event-catalyst corroboration are not required. Do not request a headline or event exhibit merely to satisfy an event workflow."
  ...
  const directionRule = evidencePack.analysisKind === "valuation"
    ? "Confirm whether current price is materially below or above a defensible valuation range. Test financial inputs, assumptions, business risks and what is already priced in. No new event is required."
    : policy.assetClass === "public_equity"

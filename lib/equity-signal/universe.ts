...
async function cachedSnapshot(cacheKey: string | null) {
  if (!cacheKey || !getR2Config().configured) return { snapshot: null as EquityUniverseSnapshot | null, etag: null as string | null };
  try {
...
export async function loadEquityUniverse(fetchImpl: typeof fetch, now = new Date()) {
  ...
  let r2Write = false;
  if (cacheKey && getR2Config().configured) {
    try {
      const result = await writeVersionedJsonToR2(cacheKey, snapshot, cached.etag ? { expectedEtag: cached.etag } : { createOnly: true });
      r2Write = result.written;
      if (result.conflict) {
        const winner = await cachedSnapshot(cacheKey);

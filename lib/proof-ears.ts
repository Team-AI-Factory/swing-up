...
export async function runHistorical(input: HistoricalRunInput = {}){
 const r2=await checkR2Health(false); const ticks=tickerList(input.tickers,input.maxTickers??3); const limit=Math.max(1,Math.min(input.maxHistoricalMatches??20,100)); const warnings=["Historical matcher only counts real stored R2/Postgres rows; no synthetic history or outcomes are generated."]; const runId=`historical-pattern-${Date.now()}`; let rawDataCount=0; let storedEventCount=0; let storedEvents: Record<string, unknown>[]=[]; let rawSignals: Record<string, unknown>[]=[]; let r2PatternCacheSaveStatus="not_attempted";
 if(process.env.DATABASE_URL){
  try{ rawDataCount=await prisma.rawDataObject.count({where:{OR:[{r2Key:{startsWith:"raw/"}},{r2Key:{startsWith:"pattern-cache/"}}]}}); }catch{ rawDataCount=0; }
 ...
 return {enabled:true,r2Available:r2.connected||r2.canRead,storedEventsAvailable:storedEventCount>0||rawDataCount>0,storedEventCount,tickersWithHistory:[...new Set(storedEvents.map(e=>String(e.ticker).toUpperCase()).filter(Boolean))],eventTypesWithHistory:[...new Set(storedEvents.map(e=>String(e.eventType)).filter(Boolean))],patternMatchesToday:cleanCount,weakSampleWarnings:[...warnings,...matches.map(m=>String(m.sampleSizeWarning||"")).filter(Boolean)],secretsRedacted:true,redactionMode:"metadata_only_safe_errors",tickersChecked:ticks,topPatternMatches:matches,patternResults:matches,rawDataObjectCount:rawDataCount,weakSampleWarningCount:weakCount,r2PatternCacheSaveStatus,stage2Locked:cleanCount===0}; }

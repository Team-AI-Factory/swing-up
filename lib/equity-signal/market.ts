...
export const MAX_ACTIONABLE_QUOTE_CACHE_AGE_MS = 15 * 60 * 1000;
...
async function fetchOne(ticker: string, fetchImpl: typeof fetch, now: Date): Promise<CachedQuote> {
  ...
  if (existing && now.getTime() - existing.lastAttemptAt < CACHE_MS) return existing;
  ...
  const result: CachedQuote = {
    // A failed refresh must never make an old quote look newly fetched. Retain
    // the original successful timestamp while recording this attempt separately.
    quoteFetchedAt: existing?.quote ? existing.quoteFetchedAt : null,
    lastAttemptAt: now.getTime(),
...
function quoteForConsumer(outcome: CachedQuote, now: Date): MarketQuote | null {
...
export async function enrichCandidateQuotes(
  ...
  const provider: ProviderResult = {
    ...
    cached: settled.some((item) => item.outcome.quoteFetchedAt !== null && now.getTime() - item.outcome.quoteFetchedAt > 1_000),
  };
  return {
    candidates,
    provider,
    marketSnapshot: settled.flatMap((item) => {
      const quote = quotesByTicker.get(item.ticker);
      return quote ? [quote] : [];
    }),
    benchmarkQuote: quotesByTicker.get(BROAD_MARKET_BENCHMARK) ?? null,

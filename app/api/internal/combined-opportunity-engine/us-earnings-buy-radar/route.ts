...
import {
  readLatestUsEarningsBuyRadar,
  runUsEarningsBuyRadar,
} from "@/lib/opportunity-engine/us-earnings-buy-radar";

export const dynamic = "force-dynamic";

const BRANCH = "agent/combined-opportunity-engine";
const state = globalThis as typeof globalThis & {
  __swingUpUsEarningsBuyRadar?: Promise<Awaited<ReturnType<typeof runUsEarningsBuyRadar>>>;
};
...
export async function GET() {
  ...
  const report = await readLatestUsEarningsBuyRadar();
...
export async function POST(request: NextRequest) {
  ...
  if (!state.__swingUpUsEarningsBuyRadar) {
    state.__swingUpUsEarningsBuyRadar = runUsEarningsBuyRadar().finally(() => {
      delete state.__swingUpUsEarningsBuyRadar;
    });
  }
  try {
    const report = await state.__swingUpUsEarningsBuyRadar;
    ...
    return NextResponse.json({
      ...
      error: error instanceof Error ? error.message.replace(/\s+/g, " ").slice(0, 400) : "earnings_buy_radar_failed",
      seriousBuys: [],
      buyCandidates: [],
      newSeriousBuys: [],

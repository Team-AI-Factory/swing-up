...
function safeError(error: unknown) {
  return error instanceof Error
    ? error.message.replace(/apikey=[^&\s]+/gi, "apikey=[redacted]").replace(/\s+/g, " ").slice(0, 260)
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    try {
      const response = await fetch(url, {
        cache: "no-store",
        headers: { accept: "application/json", "user-agent": "SwingUpGlobalScanner/2.0" },
...
function rangePosition(quote: GlobalQuote) {
  return quote.price !== null && quote.yearHigh !== null && quote.yearLow !== null && quote.yearHigh > quote.yearLow
...
function scoreCandidate(stock: GlobalStock, quote: GlobalQuote): GlobalScanCandidate {
  ...
  const opportunityPriority = clamp(
    ...
    + clamp((1 - Math.abs(position - 0.45)) * 100) * 0.15
  ...
  const riskPriority = clamp(
    ...
    + clamp(Math.max(0, 0.2 - position) * 250) * 0.25,
  );
  const yearDrawdown = quote.price !== null && quote.yearHigh !== null && quote.yearHigh > 0
    ? percentChange(quote.yearHigh, quote.price)
    : null;
  const buyResearchThemes = [
    ...(position >= 0.9 && (quote.changePercent ?? 0) >= 2 ? ["buy_breakout_momentum"] : []),
    ...(position <= 0.2 && (quote.changePercent ?? 0) >= 2 ? ["buy_oversold_recovery"] : []),
  ...
  const sellResearchThemes = [
    ...(position <= 0.1 && (quote.changePercent ?? 0) <= -2 ? ["sell_technical_breakdown"] : []),
    ...((quote.changePercent ?? 0) <= -4 && volumeRatio >= 1.5 ? ["sell_distribution_pressure"] : []),
    ...(position <= 0.2 ? ["sell_thesis_break_research"] : []),
  ];
  ...
  const reasons = [
    ...
    ...(position <= 0.15 ? ["Trading near its 52-week low"] : []),
    ...(position >= 0.9 ? ["Trading near its 52-week high"] : []),
...
function yahooSymbol(candidate: GlobalScanCandidate) {
  ...
  const usListing = country === "US" || country.includes("UNITED STATES") || ["NASDAQ", "NYSE", "AMEX"].includes(exchangeKey(candidate.exchangeShortName));
  return usListing && /^[A-Z0-9]+\.[A-Z]$/.test(candidate.symbol)
...
function parseYahooHistory(payload: unknown, symbol: string, sourceUrl: string): VerifiedHistory {
  ...
  const rows = timestamps.flatMap((timestamp, index): PriceHistoryRow[] => {
    ...
    const adjustedClose = number(adjustedCloses[index]) ?? rawClose;
    ...
    const rawHigh = number(highs[index]) ?? rawClose;
    return [{
      date: new Date(seconds * 1000).toISOString().slice(0, 10),
      close: adjustedClose,
      high: Math.max(adjustedClose, rawHigh * adjustment),
    }];
...
async function fetchYahooHistory(candidate: GlobalScanCandidate, now: Date): Promise<VerifiedHistory> {
  ...
  for (const host of ["query1.finance.yahoo.com", "query2.finance.yahoo.com"]) {
    ...
    try {
      const response = await fetch(url, {
        cache: "no-store",
        headers: { Accept: "application/json", "User-Agent": "Mozilla/5.0 (compatible; SwingUpLiveCertification/1.0)", Referer: "https://finance.yahoo.com/" },
...
export function evaluateCertifiedExtremeVolatilityHistory(
  candidate: GlobalScanCandidate,
  history: VerifiedHistory,
  now = new Date(),
): CertifiedExtremeVolatilityAlert | null {
  ...
  const alertKey = [
    CERTIFIED_EXTREME_VOLATILITY_RULE.id,
    candidate.listingKey,
    latest.date,
    Math.floor(drawdown / 5) * 5,
  ].join(":");
  return {
    ...
    reasons: [
      `Adjusted close is ${Math.abs(drawdown).toFixed(1)}% below the highest adjusted price in the last 120 sessions.`,
      `Independent external holdout observed ${CERTIFIED_EXTREME_VOLATILITY_RULE.certification.wins} wins from ${CERTIFIED_EXTREME_VOLATILITY_RULE.certification.sampleSize} signals.`,
      "Direction is uncertain, so position sizing and exit assumptions require review.",
    ],
    alertKey,
    evidence: {
      sessionsUsed: 120,
      latestMarketDate: latest.date,
      marketDataAgeDays,
      adjustedDailyHistorySource: "Yahoo Finance public chart API",
      adjustedDailyHistoryUrl: history.sourceUrl,
      independentQuoteSource: "Financial Modeling Prep",
      noSyntheticData: true,
    },
...
async function verifyCertifiedWatchOut(
  candidates: GlobalScanCandidate[],
  now: Date,
  maximumCandidates: number,
  concurrency: number,
): Promise<CertifiedVerification> {
  const prefilter = candidates
    .filter((candidate) => {
      if (candidate.price === null || candidate.price <= 0) return false;
      if (candidate.yearHigh === null || candidate.yearHigh <= 0) return true;
      return percentChange(candidate.yearHigh, candidate.price) <= -55;
    })
    .sort((left, right) => right.riskPriority - left.riskPriority);
  ...
  const results = await mapWithConcurrency(selected, concurrency, async (candidate) => {
    try {
      const history = await fetchYahooHistory(candidate, now);
      return evaluateCertifiedExtremeVolatilityHistory(candidate, history, now);
    } catch (error) {
  ...
  const coveragePercent = prefilter.length ? Number((((selected.length - failedCandidates) / prefilter.length) * 100).toFixed(2)) : 100;
  return {
    alerts,
    prefilterCandidates: prefilter.length,
    checkedCandidates: selected.length,
    failedCandidates,
    skippedCandidates,
    coveragePercent,
    coverageComplete: failedCandidates === 0 && skippedCandidates === 0,
...
export async function scanAllGlobalStocks(options?: {
  ...
  const buyResearch = [...candidates]
    .filter((candidate) => candidate.buyResearchThemes.length > 0)
    .sort((left, right) => right.opportunityPriority - left.opportunityPriority)
    .slice(0, deepQueueSize);
  const sellResearch = [...candidates]
    .filter((candidate) => candidate.sellResearchThemes.length > 0)
    .sort((left, right) => right.riskPriority - left.riskPriority)
    .slice(0, deepQueueSize);
  const watchOutResearch = [...candidates]
    .filter((candidate) => candidate.watchOutResearchThemes.length > 0)
    .sort((left, right) => right.riskPriority - left.riskPriority)
    .slice(0, deepQueueSize);
  const deepAnalysisQueue = [...new Set([...buyResearch, ...sellResearch, ...watchOutResearch, ...opportunity, ...watchOut].map((row) => row.symbol))]
    .slice(0, deepQueueSize * 3);
  ...
  return {
    ...
    seriousAlerts: {
      buy: [],
      sell: [],
      watchOut: seriousVerification.alerts,
      certifiedRuleIds: [CERTIFIED_EXTREME_VOLATILITY_RULE.id],
      verification: {
        prefilterCandidates: seriousVerification.prefilterCandidates,
        checkedCandidates: seriousVerification.checkedCandidates,
        qualifyingAlerts: seriousVerification.alerts.length,
        failedCandidates: seriousVerification.failedCandidates,
        skippedCandidates: seriousVerification.skippedCandidates,
        coveragePercent: seriousVerification.coveragePercent,
        coverageComplete: seriousVerification.coverageComplete,
        maximumCandidates: maximumCertifiedChecks,
        concurrency: certifiedCheckConcurrency,
        dataFreshnessMaximumDays: 7,
        errors: seriousVerification.errors,
      },
    },
    opportunityCoverage: opportunityCoverageSummary(),
    errors: [...new Set([...errors, ...seriousVerification.errors])].slice(0, 150),
    safety: {
      databaseWrites: false,
      publishing: false,
      notifications: false,
      seriousSignalsUnlocked: seriousVerification.alerts.length > 0,
      certifiedRuleEnabled: true,
    },
  };
}

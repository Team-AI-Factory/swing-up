...
async function runOne(sourceName: RunnableSourceName, options: Required<Pick<SourceRunOptions, "dryRun">> & SourceRunOptions): Promise<SourceRunSummaryRow> {
  ...
  try {
    if (sourceName === "GDELT") {
      ...
      const result = await runFinraShortSaleIngestion({ dryRun: options.dryRun });
      finished = finishRow(row, { status: result.ok && !result.errors.length ? "ok" : result.ok ? "degraded" : "error", recordsChecked: result.recordsChecked, signalsCreated: result.rawSignalsCreated, duplicatesSkipped: result.duplicatesSkipped, errors: result.errors, sourceHealthUpdated: sourceHealthCanPersist() });

...
async function main() {
  ...
  assert.ok(source.cases.length >= 180, `Only ${source.cases.length} clean historical cases were available; at least 180 are required.`);
  ...
  const report = {
    ...
    methodology: {
      ...
      chronologicalSplit: "Oldest 40% trained the model; next 30% selected confidence thresholds; newest 30% was untouched until one final evaluation.",
      confidenceLevel: 0.9,
      confidenceMethod: "one-sided 90% Wilson lower confidence bound",
      minimumFinalSamples,
      buySuccess: "Positive absolute return at the declared horizon; SPY excess return remains reported separately.",
  ...
  console.log(JSON.stringify({
    ...
    cleaning: report.methodology.cleaning,
...
main().catch(async (error) => {
  const report = { version: 3, passed: false, checkedAt: new Date().toISOString(), fatalError: safe(error), safety: { databaseWrites: false, publishing: false, notifications: false, payments: false, openAiCalls: false } };
  await mkdir(outputPath.split("/").slice(0, -1).join("/") || ".", { recursive: true });
  await writeFile(outputPath, `${JSON.stringify(report, null, 2)}\n`, "utf8");

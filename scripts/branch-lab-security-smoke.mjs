...
if (isolatedGet.response.status !== 404 || isolatedGet.body?.error !== "pr262_runtime_route_blocked") throw new Error(`Legacy branch lab bypassed the PR #262 controlled runtime boundary (${isolatedGet.response.status}).`);
...
if (isolatedPost.response.status !== 404 || isolatedPost.body?.error !== "pr262_runtime_route_blocked") throw new Error(`Legacy branch lab trigger bypassed the PR #262 controlled runtime boundary (${isolatedPost.response.status}).`);
...
if (untrusted.response.status !== 404 || untrusted.body?.error !== "pr262_runtime_route_blocked") throw new Error(`Public committee route bypassed the PR #262 controlled runtime boundary (${untrusted.response.status}).`);
...
if (!startScript.includes(`const PR262_BRANCH = "agent/combined-opportunity-engine"`)) throw new Error("Combined opportunity branch does not have an explicit legacy-worker rejection identity.");
for (const marker of [
...
if (pr262PauseIndex < 0 || (applicationLaunchIndex >= 0 && pr262PauseIndex > applicationLaunchIndex)) throw new Error("PR #262 legacy-worker rejection does not precede application startup.");
...
for (const marker of [`SWING_UP_BRANCH_LAB_SCHEDULER_OWNER: "dedicated_worker"`, `scripts/railway-branch-worker.mjs`, "workerLastHeartbeatAt", "dedicated worker heartbeat overdue", "WORKER_RUNTIME_STATUS_PATH", "recordWorkerStatus", "terminating supervisor so Railway can apply its restart policy", "process.exit(exitCode)"]) {
  if (!startScript.includes(marker)) throw new Error(`PR261 branch startup does not supervise its dedicated scanner: ${marker}`);
}
...
for (const marker of ["dedicated worker active", "x-swing-up-branch-lab-scheduler", "dedicated_worker", "workerStartedAt", "transport=loopback", "state=Cloudflare R2"]) {
  if (!workerSource.includes(marker)) throw new Error(`Dedicated PR261 branch scanner policy is missing: ${marker}`);
}
...
console.log(JSON.stringify({
  ok: true,
  performanceSimulationUsed: false,
  legacyAndPublicApiExecutionBlocked: true,
  protectedCronIsOnlyActivePr262ExecutionRoute: true,
  untrustedEvidenceBlocked: true,
  pr262RuntimeMode: "bounded_five_minute_cron",
  cloudflareR2PrimaryState: true,
  railwayVolumePrimaryState: false,
  pr261DedicatedWorkerPolicyStillSupervised: true,
  overdueScanWatchdog: true,
  openAiCalledBySecurityProbe: false,

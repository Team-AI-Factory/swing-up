...
import { industryLabel, publicAlertDetailsComplete } from "@/lib/alert-details";
...
export function assemblePublicSignals(candidates: Candidate[], recent: Json[], action = "all") {
  ...
  const events = recent.filter(row => {
    ...
    return {
      ...
      industry: industryLabel(row.industry, context?.industry, object(row.companyProfile).industry),
  ...
  const valuations = candidates.filter(row => row.userAlertEligible && verifiedCompanyProfile(row.companyProfile, row) && !reviewedValuations.has(row.ticker)).map(row => ({
    ...
    industry: industryLabel(row.industry, row.companyProfile.industry),
  ...
  return [...events, ...valuations].filter(row => publicAlertDetailsComplete(row))
    .filter(row => action === "all" || (action === "approved" ? row.committeeApproved : row.action === action)).sort(compareSignalPotential);
}

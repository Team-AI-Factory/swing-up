...
const analysisWithHistoricalSupport = (historicalSupport) => compile(new URL("../lib/equity-signal/analysis.ts", import.meta.url), {
  "@/lib/branch-signal-lab-policy": policy,
  "@/lib/equity-signal/historical-analogs": {
    analyzeHistoricalAnalogs: () => ({
      available: historicalSupport > 0,
      strength: historicalSupport > 0 ? "strong" : "missing",
      sampleSize: historicalSupport > 0 ? 20 : 0,
      historicalSupport,
      leakageSafe: true,
      items: [],
    }),
...
const universe = {
  ...
  entries: [
    ...
    entry("EXD", "Example Dynamics Corporation", ["Example Dynamics"]),
  ],
...
const historyInvariantReceipts = [
  receipt({
    title: "Example Dynamics CEO resigns amid sudden leadership transition",
    publisher: "Independent Business Wire",
    channel: "marketaux",
    official: false,
    primarySource: false,
  }),
  receipt({
    title: "Example Dynamics CEO resigns during unexpected leadership shakeup",
    publisher: "Independent Market News",
    channel: "gdelt",
    official: false,
    primarySource: false,
  }),
];
const noHistoryAnalysis = analysisWithHistoricalSupport(0);
const strongHistoryAnalysis = analysisWithHistoricalSupport(100);
const buildWith = (implementation, receipts) => implementation.buildImpactCandidates(receipts, universe, macro, new Date("2026-07-22T13:00:00.000Z"), []);
const marginalWithoutHistory = buildWith(noHistoryAnalysis, historyInvariantReceipts).candidates.find((item) => item.ticker === "EXD");
const marginalWithStrongHistory = buildWith(strongHistoryAnalysis, historyInvariantReceipts).candidates.find((item) => item.ticker === "EXD");
assert.equal(marginalWithoutHistory?.gateChecks.currentEvidenceStrengthPassed, false);
assert.equal(marginalWithStrongHistory?.score > marginalWithoutHistory?.score, true);
assert.equal(marginalWithoutHistory?.gatePassed, false);
assert.equal(marginalWithStrongHistory?.gatePassed, marginalWithoutHistory?.gatePassed);

const strongCurrentEvidence = [receipt({
  title: "Example Dynamics receives FDA approval for its lead product",
  channel: "openfda",
})];
const strongWithoutHistory = buildWith(noHistoryAnalysis, strongCurrentEvidence).candidates.find((item) => item.ticker === "EXD");
const strongWithHistory = buildWith(strongHistoryAnalysis, strongCurrentEvidence).candidates.find((item) => item.ticker === "EXD");
assert.equal(strongWithoutHistory?.gateChecks.currentEvidenceStrengthPassed, true);
assert.equal(strongWithoutHistory?.gatePassed, true);
assert.equal(strongWithHistory?.gatePassed, strongWithoutHistory?.gatePassed);
...
console.log(JSON.stringify({
  ...
  historicalSupportCannotChangeCurrentEvidenceGate: true,
}, null, 2));

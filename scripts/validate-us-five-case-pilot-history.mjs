...
async function main() {
  ...
  function analyze(direction) {
    const sameDirection = bootstrap.records.filter((record) => record.direction === direction);
  ...
  const report = {
    ...
    bootstrap: {
      ...
      positiveExamples: bootstrap.records.filter((record) => record.direction === "upside").length,
      negativeExamples: bootstrap.records.filter((record) => record.direction === "downside").length,
    },

...
assert.equal(health.json.researchCalibrationPolicy?.historicalComparisonRequiredForSeriousSignal, false);
assert.equal(health.json.researchCalibrationPolicy?.thisRouteCanEmitSeriousSignals, false);
assert.match(health.json.researchCalibrationPolicy?.seriousSignalPermission || "", /event_first.*full_committee/);
...
async function liveRun() {
  ...
  assert.ok(result.json.foundationDecisions.every((row) => row.blockedReasons.includes("research_only_requires_current_event_verification_price_anchor_and_full_committee")));
  assert.ok(result.json.foundationDecisions.every((row) => row.confidence?.seriousSignalEligible === false));

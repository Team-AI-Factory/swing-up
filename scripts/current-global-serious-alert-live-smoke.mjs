...
async function request(path, options = {}) {
  ...
  try {
    ...
    throw new Error(`Expected JSON from ${path}; status=${response.status}; body=${raw.slice(0, 400)}`);
  }
...
const scannerHealth = await request("/api/internal/combined-opportunity-engine/global-scan", { timeoutMs: 90_000 });
...
const scan = await request("/api/internal/combined-opportunity-engine/global-scan", {
  method: "POST",
...
assert.ok([200, 206].includes(scan.status));
...
assert.equal(scan.json?.scan?.coverageComplete, true);
assert.equal(scan.json?.seriousAlerts?.verification?.coverageComplete, true);
